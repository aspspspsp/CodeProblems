...
public class WordSearch {
    ...
    boolean dfs(char[][] board, char[] word_chars, int k, int i, int j) {
        ...
        if(i + 1 < board.length && k < word_chars.length && board[i + 1][j] == word_chars[k + 1]) //下
            isFound = isFound || dfs(board, word_chars, k + 1, i + 1, j);
        if(j + 1 < board[0].length && k < word_chars.length && board[i][j + 1] == word_chars[k + 1]) //右
            isFound = isFound || dfs(board, word_chars, k + 1, i, j + 1);
        if(j - 1 >= 0 && k < word_chars.length && board[i][j - 1] == word_chars[k + 1]) //左
            isFound = isFound || dfs(board, word_chars, k + 1, i, j - 1);
        if(i - 1 >= 0 && k < word_chars.length && board[i - 1][j] == word_chars[k + 1]) //上
            isFound = isFound || dfs(board, word_chars, k + 1, i - 1, j);
